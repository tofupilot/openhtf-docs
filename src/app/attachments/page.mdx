export const metadata = {
  title: 'Test Attachments | TofuPilot',
  description:
...
def electrical_measurements(test):
    ...
    test.attach(
        "measurement_data_csv", csv_content.encode("utf-8"), mimetype="text/csv"
    )
...
def load_attachment_phase(test):
    ...
    if attachment:
        attachment_content = attachment.data.decode(
            "utf-8"
        )  # Assuming CSV content is UTF-8 encoded
...
<Image src="/temperature_curve.png" alt="Attachement example" />
