...
When testing devices, tracking the serial number for traceability is essential. OpenHTF lets you enter the serial number manually at the start or set it programmatically at the start or during the test.

By connecting your test scripts to a dedicated database like <TofuPilotLink utmMedium="dut-overview"/>, the serial number is automatically used to organize the tests for each unit and to compare performance by component revision or batch number.
...
The <TofuPilotLink utmMedium="dut-integration" /> integration automatically uploads the OpenHTF report to a secure workspace and imports the DUT's serial number.
