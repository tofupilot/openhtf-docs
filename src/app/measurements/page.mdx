...
Measurements are essential components in OpenHTF for capturing and validating data within phases. They enable structured test result recording, apply validations, and ensure data consistency throughout the testing process. {{ className: 'lead' }}

## Simple measurement

Let's add a simple measurement to our phase. In this example, `device_status` captures a string value within `status_check_phase`.
...
@htf.measures(
    htf.Measurement("test_duration")
    .in_range(
        0, 10
    )  # Specifies that the measurement must be between 0 and 10, if the measurement is outside this range, the test will fail.
    .with_units(htf.units.SECOND)  # Sets the units of the measurement to seconds
)
def measure_duration(test):
    # Simulate capturing the duration of a test
    measured_duration = 5  # Example value for the test duration (in seconds)

    # Assign the simulated duration to the 'test_duration' measurement
    test.measurements.test_duration = measured_duration
