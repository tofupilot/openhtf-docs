...
You can use measurements to capture and validate data within phases. {{ className: 'lead' }}
...
You can use measurements in OpenHTF to capture and validate values during a test. These values can be numeric, boolean, or string, and may be either simple or multidimensional. The framework offers built-in validation, and test authors can declare measurements with units, types, and validation rules using the `@openhtf.measures()` decorator.
...
Numeric measurements represent values that need to be within a defined range or pass specific numeric validation criteria.
...
@htf.measures(
    measurements.Measurement('temperature')  # Declares a measurement 'temperature'
    .in_range(0, 100)  # Ensures the value is between 0 and 100
    .with_units('C')   # Specifies the unit as Celsius
...
<Properties>
  ...
  <Property name=".with_units(units)">
    Rounds the value to the specified precision before validation.
  </Property>
  <Property name=".with_precision(precision)">
    Rounds the value to the specified precision before validation.
  </Property>
</Properties>
