...
This example demonstrates a basic OpenHTF test that launches the Operator UI and prompts the user to enter a serial number before test start. It includes two phases: one automatic and one requiring operator input.
...
def prompt_operator_led_color(test, user_input):
    led_color = user_input.prompt(
        message="What is the LED color? (Red/Green/Blue)", text_input=True
    ) # text_input must be True for correct rendering in the Web GUI
...
def main():
    ...
    with station_server.StationServer() as server:
        ...
        test = htf.Test(hello_world, prompt_operator_led_color)
...
Use `user_input.prompt()` to ask the operator for manual input, such as confirming an LED color.
