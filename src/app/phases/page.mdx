...
Organize test flows by breaking it down into multiple phases. {{ className: 'lead' }}
...
In most cases, the phases are [measurements](/measurements) phases and setup or teardown phases with measurement equipments. 
...
Phases are standard Python functions that take the `test` object as an input argument and can either return a `PhaseResult` or a measurement outcome.
They must be individually added to the `Test` definition to be executed.
...
def main():
    ...
    test = htf.Test(setup_phase, test_voltage_phase, teardown_phase) 
