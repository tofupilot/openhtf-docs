...
You can click on a phase to compare its recent performance accross the latest runs, like the average step duration or first-pass yield.
...
You can run the phase under the Python Debugger using `run_under_pdb`. When setting this option, increase the phase `timeout_s` as well because the timeout will still apply when under the debugger.

```python {{ title: 'main.py ADD TIMEOUT!' }}
import openhtf as htf

@htf.PhaseOptions(run_under_pdb=True)
...
### Run if
You can use `run_if` callback to determine whether to execute the phase; if skipped, it won't be logged.
...
You can use `requires_state` when a phase needs to manage internal test details, such as wrapping or controlling other phases.
...
```
### Phase Groups
You can use phase groups to bundle phases inside a setup, a main and a teardown group. 
...
@htf.measures(htf.Measurement("voltage").in_range(3.2, 3.5).with_units(units.VOLT))
def second_measurement_phase(test):
    test.measurements.voltage = 3.7
