...
Test phases are a fundamental concept in OpenHTF, defined as Python functions that execute specific tasks during the test process. These phases can perform measurements, control devices, and more. {{ className: 'lead' }}
...
if __name__ == '__main__':
    ...
    test = htf.Test(pass_phase, retry_phase, conditional_fail_phase, debug_phase)
    # Execute the test
    test.execute()
```
